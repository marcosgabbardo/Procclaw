...
def create_app() -> FastAPI:
    ...
    async def trading_overview(
        ...
        """Get overview of all trade jobs with aggregated stats."""
        ...
        # Find all trade-type jobs
        trade_jobs = {}
        for job_id, job in supervisor.jobs.jobs.items():
            if job.type == JobType.TRADE:
                trade_jobs[job_id] = job

        # Recalculate stats for all trade jobs
        results = []
        totals = {
            "total_pnl": 0, "open_positions": 0, "total_trades": 0,
            "wins": 0, "losses": 0, "best_trade_pnl": 0,
            "best_trade_market": "", "best_trade_job": "",
        }

        for job_id, job in trade_jobs.items():
            initial_capital = job.trade_config.initial_capital if job.trade_config else 1000
            stats = analytics.recalculate_stats(job_id, initial_capital=initial_capital)

            # Get job status
            state = supervisor.db.get_state(job_id)
            job_status = state.status.value if state else "stopped"
            ...
            # Aggregate totals
            totals["total_pnl"] += stats.get("total_pnl", 0)
            totals["open_positions"] += stats.get("open_trades", 0)
            totals["total_trades"] += stats.get("total_trades", 0)
            totals["wins"] += stats.get("wins", 0)
            totals["losses"] += stats.get("losses", 0)

            if stats.get("best_trade_pnl", 0) > totals["best_trade_pnl"]:
                totals["best_trade_pnl"] = stats["best_trade_pnl"]
                totals["best_trade_market"] = stats.get("best_trade_market", "")
                totals["best_trade_job"] = job_id

        # Calculate overall win rate
        total_completed = totals["wins"] + totals["losses"]
    ...
    @app.get("/api/v1/trading/{job_id}/stats")
    async def trading_job_stats(
        job_id: str,
        _auth: bool = Depends(verify_token),
    ):
        """Get full stats for a single trade job."""
        from procclaw.core.trade_analytics import TradeAnalytics
        from procclaw.models import JobType
        ...
        return {"job_id": job_id, "name": job.name, **stats}

    @app.get("/api/v1/trading/{job_id}/trades")
    async def trading_job_trades(
        job_id: str,
        ...
        """Get trades for a trade job."""
        supervisor = get_supervisor()
        db_status = status if status and status != "all" else None
        trades = supervisor.db.get_trades(job_id, status=db_status, limit=limit, offset=offset)
        return {"job_id": job_id, "trades": trades, "total": len(trades)}

    @app.get("/api/v1/trading/{job_id}/decisions")
    async def trading_job_decisions(
        job_id: str,
        action: str | None = Query(None, description="buy|skip|sell"),
        limit: int = Query(100, ge=1, le=1000),
        offset: int = Query(0, ge=0),
        _auth: bool = Depends(verify_token),
    ):
        """Get decisions for a trade job."""
        supervisor = get_supervisor()
        decisions = supervisor.db.get_trade_decisions(job_id, action=action, limit=limit, offset=offset)
        return {"job_id": job_id, "decisions": decisions, "total": len(decisions)}

    @app.get("/api/v1/trading/{job_id}/equity")
    async def trading_job_equity(
        job_id: str,
        ...
        supervisor = get_supervisor()
        snapshots = supervisor.db.get_portfolio_snapshots(
            job_id, since=since, until=until, limit=limit
        )
        return {"job_id": job_id, "snapshots": snapshots, "total": len(snapshots)}

    @app.get("/api/v1/trading/{job_id}/events")
    async def trading_job_events(
        job_id: str,
        event_type: str | None = Query(None, description="scan|decision|trade_open|trade_close|portfolio"),
        limit: int = Query(100, ge=1, le=1000),
        offset: int = Query(0, ge=0),
        _auth: bool = Depends(verify_token),
    ):
        """Get raw trade events for a job."""
        supervisor = get_supervisor()
        events = supervisor.db.get_trade_events(
            job_id, event_type=event_type, limit=limit, offset=offset
        )
        # Parse data JSON for convenience
        for event in events:
            try:
                event["parsed_data"] = json.loads(event.get("data", "{}"))
            except (json.JSONDecodeError, TypeError):
                event["parsed_data"] = {}
        return {"job_id": job_id, "events": events, "total": len(events)}

    @app.get("/api/v1/trading/compare")
    async def trading_compare(
        jobs: str = Query(..., description="Comma-separated job IDs"),
        _auth: bool = Depends(verify_token),
    ):
        """Compare stats across multiple trade jobs."""
        supervisor = get_supervisor()
    ...
    @app.get("/api/v1/trading/{job_id}/export")
    async def trading_export(
        job_id: str,
        format: str = Query("json", description="json|csv"),
        what: str = Query("trades", description="trades|decisions|events"),
        _auth: bool = Depends(verify_token),
    ):
        """Export trading data as JSON or CSV."""
        from fastapi.responses import Response

        supervisor = get_supervisor()

        if what == "trades":
            data = supervisor.db.get_trades(job_id, limit=10000)
        elif what == "decisions":
            data = supervisor.db.get_trade_decisions(job_id, limit=10000)
        elif what == "events":
            data = supervisor.db.get_trade_events(job_id, limit=10000)
        else:
            raise HTTPException(status_code=400, detail=f"Unknown export type: {what}")
        ...
        if format == "csv":
            ...
            return Response(
                ...
                media_type="text/csv",
                headers={"Content-Disposition": f"attachment; filename={job_id}_{what}.csv"},
            )
        else:
