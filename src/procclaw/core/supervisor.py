        ...
        for job_type in (JobType.CONTINUOUS, JobType.TRADE):
            for job_id, job in self.jobs.get_jobs_by_type(job_type).items():
                if job.enabled:
                    ...
                    if job_id in self._processes and self._processes[job_id].is_running():
                        logger.debug(f"{job_type.value} job '{job_id}' already adopted, skipping auto-start")
                        continue
                    
                    state = self.db.get_state(job_id)
                    # Skip paused jobs
                    if state and state.paused:
                        logger.info(f"Skipping paused {job_type.value} job '{job_id}'")
                        continue
                    ...
                    logger.info(f"Auto-starting {job_type.value} job '{job_id}'")
